...
const (
	URL    = "https://api.telegram.org/bot###{Here the API Key of Telegram}###/sendMessage"
	ChatId = -100 // ID from the Channel/Group
	Opened = "opened"
)
...
func (rp *RequestPayload) String() string {
	s, _ := json.Marshal(rp)
	return string(s)
}

// Payload from the Github webook (PR only selected)
type Payload struct {
